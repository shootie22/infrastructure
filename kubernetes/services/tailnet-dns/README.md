# Private DNS for `*.infra`

This Argo application serves the `infra` zone on Fuji's Tailscale address,
...
client can run `dig @100.64.0.1 hl.infra A` and receive `100.64.0.1`. Only
then add `dns.nameservers.split.infra: [100.64.0.1]` to Headscale's config.
Do not point clients at this resolver until these checks pass: failed DNS
would affect the entire `infra` suffix.

The `hl.infra` DNS record currently names Fuji only. Headlamp still requires
...
add `mon.infra` or `gp.infra` only after their private HTTP endpoints exist.
Ordinary publicly trusted ACME certificates are unavailable for `.infra`.
